module LCD_B(rst,clk,LCD_E,LCD_RS,LCD_RW,LCD_DATA,LED_out,number_btn,control_btn,line_control);

input rst,clk;
input [9:0] number_btn;
input [1:0]control_btn;
input line_control;//line1,2 전환을 위한 input
...
begin
    ...
    begin
        case(state)
            ...
            SET_ADDRESS:
                if (line_control) begin
                   {LCD_RS,LCD_RW,LCD_DATA} <= 10'b0_0_1100_0000; // 버튼 누르면 line2로 이동
            end else begin
              {LCD_RS,LCD_RW,LCD_DATA} <= 10'b0_0_1000_0000; // 버튼 안누르면 line1으로 이동
            end               
            DELAY_T:
                {LCD_RS,LCD_RW,LCD_DATA}<=10'b0_0_0000_1111;                
            WRITE:begin
